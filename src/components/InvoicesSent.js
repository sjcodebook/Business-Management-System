...
const InvoicesSent = () => {
  ...
  const setInvoicesSentData = async (resultLimit) => {
    try {
      setIsLoading(true)
      let requests = await getUnpaidInvoicesSent(resultLimit)
        ...
        if (isNumeric(searchQuery)) {
          ...
          await Promise.all(
            clientsData.map(async (clientData) => {
              let invoicesSent = await getUnpaidInvoicesSentByClientId(clientData.id, limit)
              invoicesSent.forEach((request) => {
                request['allInfo'] = JSON.parse(request.allInfo)
  ...
  const fetchCSVData = async () => {
    try {
      setIsLoading(true)
      let invoices = await getUnpaidInvoicesSentByThreshold(fromDateUnix, toDateUnix)
      await Promise.all(
        invoices.map(async (invoice) => {
          if (invoice.clientId) {
            let client = await getCleintById(invoice.clientId)
      ...
      let finalCSVData = invoices.map((i) => {
        return {
          ...
          'Sent On': dayjs.unix(i?.emailSentAt).format('DD MMMM YYYY'),
    ...
    <div className={classes.root}>
      ...
      {showCSVDownloadDrawer && (
        <DownloadCSVModal
          ...
          isLoading={isLoading}
          title='Download Sent Invoices CSV'
          fileName='Sent Invoices'
        />
      )}
      ...
      if (confirm(`Are You Sure to delete ${selectedRecords.length} unpaid invoice(s)?`)) {
        ...
        await Promise.all(
          selectedRecords.map(async (recordId) => {
            ...
            let invoice = invoicesSent.find((invoice) => invoice.id === recordId)
  ...
  return (
    <Paper>
      ...
      <Typography variant='body1' color='textSecondary' align='center' gutterBottom>
        ...
        Total Results: {invoicesSent.length}
            ...
            <TableHead>
              <TableRow>
                ...
                <TableCell align='center' padding='checkbox'>
                  <Checkbox
                    indeterminate={
                      selectedRecords.length !== 0 && selectedRecords.length !== invoicesSent.length
                    }
                    checked={
                      invoicesSent.length !== 0 && selectedRecords.length === invoicesSent.length
                    }
                    onChange={(e) => {
                      if (selectedRecords.length !== 0) {
                        setSelectedRecords([])
                      } else {
                        setSelectedRecords(invoicesSent.map((rec) => rec.id))
                      }
                    }}
                  />
                </TableCell>
