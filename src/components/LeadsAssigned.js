...
const LeadsAssigned = () => {
  ...
  const setLeadsAssignedData = async (resultLimit) => {
    try {
      setIsLoading(true)
      let leads = await getLeadsAssigned(resultLimit)
      await Promise.all(
      ...
      setIsLoading(false)
      setLeadsAssigned(leads)
    } catch (err) {
      setIsLoading(false)
      console.error(err)
      showToast('Something went wrong fetching leads assigned', 'error')
    }
  }
  ...
  const handleSearch = async (searchQuery) => {
    try {
      if (searchQuery !== '' && allowSearch) {
        ...
        setIsLoading(false)
        setLeadsAssigned(finalRequests)
      }
    } catch (err) {
      setIsLoading(false)
      console.error(err)
      ...
      <Container fixed>
        ...
        {showCSVDownloadDrawer && (
          <DownloadCSVModal
            ...
            isLoading={isLoading}
            title='Download Leads Assigned CSV'
            fileName='Leads Assigned'
          />
        )}
      </Container>
    </div>
      ...
      if (confirm(`Are You Sure to delete ${selectedRecords.length} record(s)?`)) {
        ...
        await Promise.all(
          selectedRecords.map(async (recordId) => {
            ...
            let request = leadsAssigned.find((request) => request.id === recordId)
    ...
    <Paper>
      ...
      <Typography variant='body1' color='textSecondary' align='center' gutterBottom>
        ...
        {selectedRecords.length !== 0 ? (
          ...
          <>Total Results: {leadsAssigned.length}</>
        )}
      </Typography>
      <br />
              ...
              <TableRow>
                ...
                <TableCell align='center' padding='checkbox'>
                  <Checkbox
                    indeterminate={
                      selectedRecords.length !== 0 &&
                      selectedRecords.length !== leadsAssigned.length
                    }
                    checked={
                      leadsAssigned.length !== 0 && selectedRecords.length === leadsAssigned.length
                    }
                    onChange={(e) => {
                      if (selectedRecords.length !== 0) {
                        setSelectedRecords([])
                      } else {
                        setSelectedRecords(leadsAssigned.map((rec) => rec.id))
                      }
                    }}
                  />
                </TableCell>
