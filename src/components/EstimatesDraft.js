...
const EstimatesDraft = () => {
  const classes = useStyles()
  const theme = useTheme()
  const isMobile = useMediaQuery(theme.breakpoints.down('sm'))
  const [estimatesDraft, setEstimatesDraft] = useState([])
  ...
  const setEstimatesDraftData = async (resultLimit) => {
    try {
      setIsLoading(true)
      let requests = await getEstimatesDraft(resultLimit)
        ...
        if (isNumeric(searchQuery)) {
          ...
          await Promise.all(
            clientsData.map(async (clientData) => {
              let estimatesDraft = await getEstimatesDraftByClientId(clientData.id, limit)
              estimatesDraft.forEach((request) => {
                request['allInfo'] = JSON.parse(request.allInfo)
          ...
          setEstimatesDraft(finalRequests)
        }
        setIsLoading(false)
      }
    } catch (err) {
  ...
  const fetchCSVData = async () => {
    try {
      setIsLoading(true)
      let estimates = await getEstimatesDraftByThreshold(fromDateUnix, toDateUnix)
      await Promise.all(
        estimates.map(async (estimate) => {
          if (estimate.clientId) {
            let client = await getCleintById(estimate.clientId)
      ...
      let finalCSVData = estimates.map((e) => {
        return {
          ...
          'Drafted On': dayjs.unix(e?.createdAt).format('DD MMMM YYYY'),
        }
      })
    ...
    <div className={classes.root}>
      ...
      {showCSVDownloadDrawer && (
        <DownloadCSVModal
          ...
          isLoading={isLoading}
          title='Download Draft Estimates CSV'
          fileName='Draft Estimates'
        />
      )}
    </div>
  )
  ...
  const handleMoveToSent = async () => {
    try {
      if (
        ...
        confirm(
          `Are You Sure to move ${selectedRecords.length} record(s) to ${Constants.EstimateStatus['NONE']}?`
        )
      ) {
        ...
        await Promise.all(
          selectedRecords.map(async (recordId) => {
            await changeEstimateEmailStatus(recordId, true)
          })
        )
        setSelectedRecords([])
        setRefresh((prevVal) => {
    ...
    <Paper>
      ...
      <Typography variant='body1' color='textSecondary' align='center' gutterBottom>
        ...
        {selectedRecords.length !== 0 ? (
          ...
          <span>Total Results: {estimatesDraft.length}</span>
        )}
      </Typography>
      <br />
              ...
              <TableRow>
                ...
                <TableCell align='center' padding='checkbox'>
                  <Checkbox
                    indeterminate={
                      selectedRecords.length !== 0 &&
                      selectedRecords.length !== estimatesDraft.length
                    }
                    checked={
                      estimatesDraft.length !== 0 &&
                      selectedRecords.length === estimatesDraft.length
                    }
                    onChange={(e) => {
                      if (selectedRecords.length !== 0) {
                        ...
                        setSelectedRecords(estimatesDraft.map((rec) => rec.id))
                      }
                    }}
                  />
                </TableCell>
