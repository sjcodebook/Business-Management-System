...
export default function ToolsImagesModal({ workKey, valueId, onClose }) {
...
const Dropzone = ({ startUpload, setStartUpload, onClose, files, setFiles, workKey, valueId }) => {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (opStatus === 'complete' && !files[currFileIndex + 1]) {
        estimatorStore.addToolsImages(workKey, valueId, imagesUrl)
      ...
      {startUpload ? (
        ...
        <>
          <div style={{ margin: 40 }}>
            ...
            <br />
            <Scrollbars style={{ height: files.length !== 0 ? 150 : 0 }}>
  ...
  const uploadImageAsPromise = async (imageFile) => {
    try {
      ...
      return new Promise(function (resolve, reject) {
        setOpStatus('running')
        // references
        const storageRef = storage.ref('ToolsImages/' + newImageFile.name)
