...
const SaleLostEstimates = () => {
  const classes = useStyles()
  const theme = useTheme()
  const isMobile = useMediaQuery(theme.breakpoints.down('sm'))
  const [estimatesLost, setEstimatesLost] = useState([])
  ...
  const setEstimatesLostData = async (resultLimit) => {
    try {
      setIsLoading(true)
      let requests = await getEstimatesLost(resultLimit)
      ...
      if (searchQuery !== '' && allowSearch) {
        ...
        if (isNumeric(searchQuery)) {
          ...
          estimates = estimates.filter(
            (estimate) => estimate.emailSent && estimate.saleStatus === 'SALE_LOST'
          )
          ...
          await Promise.all(
            clientsData.map(async (clientData) => {
              let estimatesSent = await getEstimatesLostByClientId(clientData.id, limit)
          ...
          setEstimatesLost(finalRequests)
        }
        setIsLoading(false)
      }
    } catch (err) {
  ...
  const fetchCSVData = async () => {
    try {
      setIsLoading(true)
      let estimates = await getEstimatesLostByThreshold(fromDateUnix, toDateUnix)
      await Promise.all(
        estimates.map(async (estimate) => {
          let client = await getCleintById(estimate.clientId)
          if (client.exists) {
      ...
      let finalCSVData = estimates.map((e) => {
        return {
          ...
          'Lost On': dayjs.unix(e?.saleStatusUpdateAt).format('DD MMMM YYYY'),
        }
      })
    ...
    <div className={classes.root}>
        ...
        <Grid container spacing={1}>
          ...
          <Grid item xs={12}>
            <Paper className={classes.paper}>
              ...
              <Grid container spacing={1}>
                <Grid item xs={12}>
                  <SaleLostEstimatesTable
                    estimatesLost={estimatesLost}
      ...
      {showCSVDownloadDrawer && (
        <DownloadCSVModal
          ...
          isLoading={isLoading}
          title='Download Lost Estimates CSV'
          fileName='Lost Estimates'
        />
      )}
    </div>
  )
      ...
      if (
        ...
        await Promise.all(
          selectedRecords.map(async (recordId) => {
            ...
            let moveEvent
            if (status === 'SOLD') {
              moveEvent = Constants.Events.ESTIMATE_MOVE_FROM_LOST_TO_SOLD
            } else if (status === 'NONE') {
              moveEvent = Constants.Events.ESTIMATE_MOVE_FROM_LOST_TO_SENT
            } else if (status === 'SCHEDULED') {
              moveEvent = Constants.Events.ESTIMATE_MOVE_FROM_LOST_TO_SCHEDULED
            }
            let targetType = moveEvent.Type
            let eventDesc = moveEvent.Desc
            let byId = userStore.currentUser.id
            let estimate = estimatesLost.find((estimate) => estimate.id === recordId)
    ...
    <Paper>
      ...
      <Typography variant='body1' color='textSecondary' align='center' gutterBottom>
        ...
        {selectedRecords.length !== 0 ? (
          ...
          <span>Total Results: {estimatesLost.length}</span>
        )}
      </Typography>
      <br />
              ...
              <TableRow>
                ...
                <TableCell align='center' padding='checkbox'>
                  <Checkbox
                    indeterminate={
                      selectedRecords.length !== 0 &&
                      selectedRecords.length !== estimatesLost.length
                    }
                    checked={
                      estimatesLost.length !== 0 && selectedRecords.length === estimatesLost.length
                    }
                    onChange={(e) => {
                      if (selectedRecords.length !== 0) {
                        setSelectedRecords([])
                      } else {
                        setSelectedRecords(estimatesLost.map((rec) => rec.id))
                      }
                    }}
                  />
                </TableCell>
...
export default SaleLostEstimates
